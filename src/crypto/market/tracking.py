...
async def manage_orders(mexc_client: MexcClient, kucoin_client: KucoinClient, database_client: DatabaseClient):
    ...
    if full_rmv_balance - INVENTORY_BALANCE > 20_000: # we are long
        ask_shift -= MEXC_TICK_SIZE
        bid_shift -= MEXC_TICK_SIZE
    elif full_rmv_balance - INVENTORY_BALANCE < -20_000: # we are short
        bid_shift += MEXC_TICK_SIZE
        ask_shift += MEXC_TICK_SIZE
    elif full_rmv_balance - INVENTORY_BALANCE > 35_000: # we are long
        ask_shift -= 2 * MEXC_TICK_SIZE
        bid_shift -= 2 * MEXC_TICK_SIZE
    elif full_rmv_balance - INVENTORY_BALANCE < -35_000: # we are short
        bid_shift += 2 * MEXC_TICK_SIZE
        ask_shift += 2 * MEXC_TICK_SIZE
